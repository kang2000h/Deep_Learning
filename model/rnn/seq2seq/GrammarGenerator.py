...
def create_model(session, forward_only):
    ...
    enc_dict, dec_dict = load_vocab("./final/Encoder_Dict_key2val_file_forGG", "./final/Decoder_Dict_val2key_file_forGG")
    ...
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        ...
        model.saver.restore(session,ckpt.model_checkpoint_path)  # 이미 checkpoint 존재하면 로드, 즉 모델이 저장되어있으면 그래프 초기화를 하지않아도 그래프를 가져올 수 있다.
    else:
...
def train():
    with tf.Session() as sess:
        ...
        _, _, _, _, enc_inputs,dec_inputs,enc_vocab,rev_dec_vocab =load_data_v2("./final/data_recent.csv",
...
def decode(sess, model, query, isTest):
    ...
    '''
    # Load vocabularies.
    enc_vocab, rev_dec_vocab = load_vocab("./final/Encoder_Dict_key2val_file_forGG", "./final/Decoder_Dict_val2key_file_forGG")
