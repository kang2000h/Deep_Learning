...
def zero_centered(inputs):
    MEAN = np.mean(np.mean(inputs, axis=0), axis=0)
    return inputs-MEAN
...
def train_and_val(tr_X, tr_Y, val_X, val_Y, batch_size):
    # get train and test data
    # return Acc

    # Get the number of training/validation steps per epoch
    train_batches_per_epoch = int(np.floor(len(tr_X) / batch_size))
    val_batches_per_epoch = int(np.floor(len(val_X) / batch_size))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            ...
            for step in range(train_batches_per_epoch):
                ...
                batch_mask = np.random.choice(len(tr_X), batch_size)
                batch_xs = tr_X[batch_mask]
                batch_ys = tr_Y[batch_mask]

                # And run the training op
                tacc, _, loss_ = sess.run([accuracy, train_op, loss], feed_dict={x: batch_xs,
                                                                                 y: batch_ys,
            ...
            for i in range(val_batches_per_epoch + 1):
                batch_xs = val_X[i * batch_size:i * batch_size + batch_size]
                batch_ys = val_Y[i * batch_size:i * batch_size + batch_size]
...
for train_indices, test_indices in k_fold.split(all_imgs, val_labels_onehot):
    ...
    tr_X = all_imgs[train_indices,:,:,:]
    tr_Y = val_labels_onehot[train_indices,:]
    val_X = all_imgs[test_indices,:,:,:]
    val_Y = val_labels_onehot[test_indices,:]

    test_Acc, conf_matrix = train_and_val(tr_X, tr_Y, val_X, val_Y, batch_size)
    total_test_Acc += test_Acc
    total_conf_matrix.append(conf_matrix)

    count+=1

print("Average_Acc_onKfold_CV", float(total_test_Acc/num_K))
for cm in total_conf_matrix:
    print(cm)
'''
