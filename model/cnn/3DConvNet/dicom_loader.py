...
def dicom_loader(save_filename):
...
def main():
    ...
    # data_dir = './data' # required arg
    # class_list = ['CE','LAA'] # required arg, director# y list
    # save_filename = './tmp/neurology.npy'# required arg

    if os.path.isfile(args.save_filename) is True:
        print("that filename is already stored!")
    else:
        # we will save big list array into a file.
        much_data = []  # list of class data

        for num, each_class in enumerate(args.class_list):
            each_class_data = []
            for num, each_patient in enumerate(os.listdir(os.path.join(args.data_dir, each_class))):
                if num % 100 == 0:
                    print(num)
                try:
                    img_data = process_data(os.path.join(args.data_dir, each_class, each_patient), args.img_px_size, args.slices)
        ...
        np.save(args.save_filename, np.array(much_data))
