...
def viewer(samples, rows=5, cols=6, show_every=3):
    fig = plt.figure()
    for num, each_slice in enumerate(samples):
        num+=1
        if num/show_every > rows*cols:
            break
        if num%show_every==0:
            print(num, show_every)
            y = fig.add_subplot(rows, cols, num/show_every)  # it will be [3, 4] sub plot grid
...
def main():
    ...
    imgs_to_process = np.load(os.path.join(args.output_path, args.save_filename))
